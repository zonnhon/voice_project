...
TTS_BASE_URL = os.getenv("TTS_BASE_URL", "http://localhost:5000").rstrip("/")
TTS_TIMEOUT_SEC = 60
...
def call_tts_api_to_wav(text: str, out_wav_path: str) -> None:
    """
    等效於：
    curl -X POST -H 'Content-Type: application/json' \
      -d '{ "text": "This is a test." }' -o test.wav \
      localhost:5000
    假設服務回傳 audio/wav bytes
    """
    url = f"{TTS_BASE_URL}"  # 你的例子是直接打 root，如果你的端點是 /tts 就改成 f"{TTS_BASE_URL}/tts"
    headers = {"Content-Type": "application/json"}
    payload = {"text": text}

    r = requests.post(url, headers=headers, json=payload, timeout=TTS_TIMEOUT_SEC)
    r.raise_for_status()
...
def play_wav(wav_path: str) -> None:
    ...
    讀 wav 檔並播放
    """
    audio, sr = sf.read(wav_path, dtype="float32", always_2d=True)  # shape (N, C)
                ...
                try:
                    ...
                    if not text:
                        ...
                        try:
                            ...
                            try:
                                ts = time.strftime("%Y%m%d_%H%M%S")
                                tts_wav_path = os.path.join(OUTPUT_DIR, f"tts_{ts}_{seg_count:03d}.wav")
                                call_tts_api_to_wav(reply, tts_wav_path)
                                print(f"🔊 TTS wav saved: {tts_wav_path}")

                                play_wav(tts_wav_path)
                            except requests.RequestException as e:
                                print("  TTS API error:", str(e))
                            except Exception as e:
                                print("  Play audio error:", str(e))
